...
contract VendingMachineFuzzTest is Test {
    ...
    function testFuzz_LoadMultipleTracks(
        uint8[] memory trackIds,
        string[] memory names,
        ...
        for (uint i = 0; i < trackIds.length; i++) {
            products[i] = IVendingMachine.Product({
                name: names[i],
                imageURI: string(abi.encodePacked("uri_", i))
            });
        }
    ...
    function testFuzz_RestockTrack(
        uint8 trackId,
    ...
    function testFuzz_VendFromTrack(
        uint8 trackId,
        uint256 price,
        address recipient,
        uint256 userBalance
    ...
    function testFuzz_VendWithZeroRecipient(
        uint8 trackId,
        uint256 price
    ) public {
        vm.assume(trackId < NUM_TRACKS);
    ...
    function testFuzz_WithdrawRevenue(
        address token,
        address to,
        uint256 amount,
    ...
    function testFuzz_StockBoundaries(
        uint8 trackId,
        uint256 stock
    ) public {
        vm.assume(trackId < NUM_TRACKS);
    ...
    function testFuzz_InvariantMaxStock(
        uint8 trackId,
